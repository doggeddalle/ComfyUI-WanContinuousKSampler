// Canvas widget for the consolidated Wan SCAIL-2 Identity Control node.
...
// matching the model's trained palette. Markers serialise into the hidden
// "markers" STRING widget as {"reference":[...], "driving":[...]}.

import { app } from "../../scripts/app.js";
import { api } from "../../scripts/api.js";

// Must match SCAIL_PALETTE in this pack's scail_core.py.
...
function setupNode(node) {
    ...
    function updateWarning() {
        ...
        if (!autoDetect && refN < drvN) {
            warningDiv.textContent = `⚠ ${refN} reference vs ${drvN} driving subject(s): ${drvN - refN} driving subject(s) have no reference to map to (auto-detect is off).`;
        } else {
    ...
    function normalizeBox(b) {
        ...
        const img = activeImg();
        if (!img) return;
        b.x = Math.max(0, Math.min(img.naturalWidth - 1, b.x));
        b.y = Math.max(0, Math.min(img.naturalHeight - 1, b.y));
        b.w = Math.max(1, Math.min(img.naturalWidth - b.x, b.w));
        b.h = Math.max(1, Math.min(img.naturalHeight - b.y, b.h));
    }
    ...
    node._wanScailOnExecuted = (message) => {
    ...
    node._wanScailSync = () => { syncFromWidget(); redraw(); };
...
app.registerExtension({
    name: "wan22.scailIdentityControl",
    async beforeRegisterNodeDef(nodeType, nodeData) {
        if (nodeData.name !== "WanSCAILIdentityControl") return;
        ...
        nodeType.prototype.onExecuted = function (message) {
            ...
            this._wanScailOnExecuted?.(message);
        };
